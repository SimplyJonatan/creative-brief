...
def scrape_top_ads(page: Page, assets_dir: Path, max_ads: int = 10) -> list:
    ...
    try:
        ...
        for selector in ["[class*='cookie'] button", "[class*='consent'] button", "[class*='accept']"]:
            try:
        ...
        # Find ad cards
        ad_cards = page.query_selector_all("[class*='card'], [class*='Card'], [class*='item'], [class*='Item']")
        print(f"[TikTok] Found {len(ad_cards)} potential ad cards")
        ...
        for i, card in enumerate(ad_cards[:max_ads]):
            try:
                ...
                # Screenshot the card
                img_path = tiktok_dir / f"top_ad_{i}.png"
                card.screenshot(path=str(img_path))

                results.append({
                    "id": f"tiktok_top_{i}_{int(time.time())}",
                    "platform": "TikTok",
                    "brand": "TikTok Top Ad",
